     ...
     char *argv[];
{
  ...
  while ((c = getopt_long (argc, argv, "yvdhrltknVL:o:b:p:", longopts, (int *)0))
	 != EOF)
    {
      switch (c)
	{
	...
	case 'L':
	  if (strcasecmp(optarg, "c") == 0) {
	    langflag = 0;
		     ...
		     (strcasecmp(optarg, "perl") == 0)) {
	    langflag = 1;
	  } else {
	    fprintf(stderr, "%s: invalid language: must be c or perl\n",
		    program_name);
	    exit(1);
	  }
	  break;
