...
#ifndef XPFILE0_
#define XPFILE0_ "GNU_BISON:[000000]BISON.SIMPLE"
#endif
#ifndef XPFILE0_1
#define XPFILE0_1 "GNU_BISON:[000000]BISON.HAIRY"
#endif
#ifndef XPFILE1_
#define XPFILE1_ "GNU_BISON:[000000]BISON.SIMPLE.PERL"
#endif
#ifndef XPFILE1_1
#define XPFILE1_1 "GNU_BISON:[000000]BISON.HAIRY.PERL"
#endif
#endif
...
openfiles()
{
  ...
  if (spec_outfile)
	...
	if (!strcmp (name_base + base_length - 2, ".pl"))
	  base_length -= 3;
      }
  ...
  else
    {
	 ...
	 or use y.tab.c (y.tab.pl), etc., if -y was specified.  */
  ...
  if (! noparserflag) 
    {
      ...
      if (filename == 0 && cp != NULL)
        {
	  if (langflag == 0)
	    filename = xmalloc(strlen(cp) + strlen(PFILE0_) + 2);
	  else
	    filename = xmalloc(strlen(cp) + strlen(PFILE1_) + 2);
          strcpy(filename, cp);
...
#endif /* MSDOS */
      if (langflag == 0)
	fparser = tryopen(filename ? filename : PFILE0_, "r");
      else
	fparser = tryopen(filename ? filename : PFILE1_, "r");
    }
  ...
  if (definesflag)
	...
	defsfile = stringappend(name_base, base_length, ".ph");
  ...
  if (spec_outfile)
    ...
    if (langflag == 0) 
      ...
      tabfile = stringappend(name_base, base_length, ".pl");
  ...
  if (langflag == 0) {
    ...
    attrsfile = stringappend(name_base, short_base_length, "_stype.ph");
    guardfile = stringappend(name_base, short_base_length, "_guard.pl");
  }
#else
#ifdef MSDOS
  ...
  if (langflag == 0)
    ...
    guardfile = stringappend(name_base, short_base_length, ".gup");
#else
  if (langflag == 0) {
    attrsfile = stringappend(name_base, short_base_length, ".stype.h");
    guardfile = stringappend(name_base, short_base_length, "_guard.c");
  } else {
    attrsfile = stringappend(name_base, short_base_length, ".stype.ph");
    guardfile = stringappend(name_base, short_base_length, "_guard.pl");
  }
#endif /* not MSDOS */
#endif /* not VMS */
  ...
  if (! noparserflag) 
    {
      ...
      if (filename == 0 && cp != NULL)
        {
	  if (langflag == 0)
	    filename = xmalloc(strlen(cp) + strlen(PFILE0_1) + 2);
	  else
	    filename = xmalloc(strlen(cp) + strlen(PFILE1_1) + 2);
          strcpy(filename, cp);
          cp = filename + strlen(filename);
          *cp++ = '/';
          strcpy(cp, langflag == 0 ? PFILE0_1 : PFILE1_1);
        }
#endif
      fparser= tryopen(cp = (filename ? filename 
		                : (langflag == 0 ? PFILE0_1 : PFILE1_1)), "r");
    }
