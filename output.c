...
#define lang_choice(opt1, opt2) ((langflag == 0) ? (opt1) : (opt2))
...
#define	ACTSTR_perl	"\nlocal($yyval);\
\nlocal($yychar);\n\n\
sub yyaction ($$$) {\n  my ($yyn,$yyvsp,$yylsp) = @_;\n\
  $yyerror = 0;\n$yycost = 0;\n  $yymsg = '';\n  if (0) {\n"

#define	ACTSTR_SIMPLE	"\n  switch (yyn) {\n"

#define	ACTSTR_SIMPLE_perl	"\n  if (0) {\n"

void
output_headers()
{
  if (semantic_parser)
    fprintf(fguard, lang_choice(GUARDSTR, GUARDSTR_perl), attrsfile);
  ...
  if (langflag == 0)
    ...
    fprintf(faction, (semantic_parser ? ACTSTR_perl : ACTSTR_SIMPLE_perl), attrsfile);
...
output()
{
  ...
  if (semantic_parser && (langflag == 0))
    fprintf(ftable, "#include \"%s\"\n", attrsfile);

  if (! noparserflag && (langflag == 0))
    fprintf(ftable, "#include <stdio.h>\n\n");
...
#define out_lang_var(ctype,name) lang_choice(ctype##" "##name##"[]", "@"##name)
#define out_lang_blist lang_choice("{", "(")
#define out_lang_elist lang_choice("}", ")")
...
/*   register short *sp; JF unused */
  ...
  if (translations)
	...
	fprintf(ftable,
	       "\n#define YYTRANSLATE(x) ((x) <= %d ? $yytranslate[x] : %d)\n",
		max_user_token_number, nsyms);
    
      if (ntokens < 127)  /* play it very safe; check maximum element value.  */
	fprintf(ftable, "\n%s = %s     0",
		out_lang_var("static const char", "yytranslate"),
		out_lang_blist);
      else
	fprintf(ftable, "\n%s = %s     0",
		out_lang_var("static const short", "yytranslate"),
		out_lang_blist);
...
output_gram()
{
  ...
  fprintf(ftable, "\n%s = %s     0",
	  out_lang_var("static const short", "yyprhs"),
	  out_lang_blist);
  ...
  fprintf(ftable, "\n%s = %s%6d",
	  out_lang_var("static const short", "yyrhs"),
	  out_lang_blist,
	  ritem[0]);
...
output_stos()
{
  ...
  fprintf(ftable, "\n%s = %s     0",
	  out_lang_var("static const short", "yystos"),
	  out_lang_blist);
...
output_rule_data()
{
  ...
  fprintf(ftable, "\n%s = %s 0",
	  out_lang_var("static const short", "yyrline"),
	  out_lang_blist);
  ...
  /*
  fprintf(ftable, "\n%s = %s   %c%s%c",
	  out_lang_var("static const char * const", "yytname"),
	  out_lang_blist,
	  lang_choice('"','\''), tags[0], lang_choice('"','\''));
	  */
  ...
  for (i = 1; i < nsyms; i++)
    ...
    fprintf(ftable, ", %s\n%s;\n", 
	    lang_choice("NULL", "'NULL'"),
	    out_lang_elist);	/* add a NULL entry to list of tokens */
  ...
  if (toknumflag) 
    {
      ...
      fprintf(ftable, "\n%s = %s 0",
	      out_lang_var("static const short", "yytoknum"),
	      out_lang_blist);
  ...
  fprintf(ftable, "\n%s = %s     0",
	  out_lang_var("static const short", "yyr1"),
	  out_lang_blist);
  ...
  fprintf(ftable, "\n%s = %s     0",
	  out_lang_var("static const short", "yyr2"),
	  out_lang_blist);
  ...
  fprintf(ftable, "%6d\n%s;\n", nitems - rrhs[nrules] - 1, out_lang_elist);
  FREE(rrhs + 1);
}
...
token_actions()
{
  ...
  fprintf(ftable, "\n%s = %s%6d",
	  out_lang_var("static const short", "yydefact"),
	  out_lang_blist,
	  k);
...
goto_actions()
{
  ...
  fprintf(ftable, "\n%s = %s%6d",
	  out_lang_var("static const short", "yydefgoto"),
	  out_lang_blist,
	  k);
...
output_base()
{
  ...
  fprintf(ftable, "\n%s = %s%6d",
	  out_lang_var("static const short", "yypact"),
	  out_lang_blist,
	  base[0]);
  ...
  fprintf(ftable, "\n%s = %s%6d",
	  out_lang_var("static const short", "yypgoto"),
	  out_lang_blist,
	  base[nstates]);
...
output_table()
{
  ...
  fprintf(ftable, "\n%s = %s%6d",
	  out_lang_var("static const short", "yytable"),
	  out_lang_blist,
	  table[0]);
...
output_check()
{
  ...
  fprintf(ftable, "\n%s = %s%6d",
	  out_lang_var("static const short", "yycheck"),
	  out_lang_blist,
	  check[0]);
...
#if 0	/* JF no longer needed 'cuz open_extra_files changes the
  ...
  while (1)
	...
	if (write_line)
	  if (c == '$')
		...
		c = getc(fpars);
		if (c == '!') {
		  /* `$%!' in the parser file indicates where to put actions.
