 ...
 */
  ...
  if (langflag == 0) {
    ...
    if (noparserflag) 
      ...
      fprintf(ftable, "\n#  A Bison parser, made from %s\n", infile);
    fprintf(ftable, "#  by  %s\n\n", version_string);
    fprintf(ftable, "# Identify Bison output.\n");
    fprintf(ftable, "#define YYBISON 1\n\n");
  }
...
{
  if (yylsp_needed)
    fprintf(f, (langflag == 0) ? LTYPESTR : LTYPESTR_perl);
}
...
void
copy_perl_varsub(fp, rule, stack_offset)
FILE* fp;
symbol_list *rule;
int stack_offset;
{
  ...
  if (c == '$')
	...
	type_name = get_type_name(n, rule);
      ...
      fprintf(fp, "$yyvs[$yyvsp+%d]", n - stack_offset);
      if (type_name)
	fprintf(fp, "->{%s}", type_name);
      if(!type_name && typed)	
...
void
copy_c_varsub(fp, rule, stack_offset)
FILE* fp;
symbol_list *rule;
int stack_offset;
{
  ...
  if (c == '$')
	...
	type_name = get_type_name(n, rule);
      ...
      fprintf(fp, "yyvsp[%d]", n - stack_offset);
      if (type_name)
	fprintf(fp, ".%s", type_name);
...
void
copy_quote(fp, match, subst, rule, stack_offset)
FILE* fp;
char match;
int subst;
symbol_list *rule;
...
int stack_offset;
{
  ...
  c = getc(finput);
  if (c == '@') { /* Modified to use @@N instead of @N */
    c = getc(finput);
    if (isdigit(c) || c == '-')
      {
	ungetc (c, finput);
    ...
    if (langflag == 0)
      fprintf(fp, "yylsp[%d]", n - stack_offset);
    else
      fprintf(fp, "$yyls[$yylsp+%d]", n - stack_offset);
    yylsp_needed = 1;
    
    ungetc(c, finput);
  } else { /* Only one @, so normal perl '@' */
    putc('@', fp);
    ungetc(c, finput);
  }
}
...
copy_definition ()
{
  ...
  for (;;)
	...
	case '/':
	  ...
	  putc(c, fattrs);
	  if (c == '/')
	    copy_line_comment(fattrs, NULL);
	  else
	    copy_c_comment(fattrs, NULL);
  ...
  while (c != EOF)
	...
	case '/':
	  ...
	  if (c != '*' && c != '/')
	    ...
	    if (c == '/')
	      copy_line_comment(fattrs, fdefines);
	    else
	      copy_c_comment(fattrs, fdefines);
...
int stack_offset;
{
  ...
  while (brace_flag ? (count > 0) : (c != ';'))
	...
	case '\'':
	case '"':
	  copy_quote(fguard, c, 0, rule, stack_offset);
	  break;
	...
	case '/':
	  ...
	  if (c == '/')
	    copy_line_comment(fguard, NULL);
	  else
	    copy_c_comment(fguard, NULL);

	  break;

	case '$':
	  if (langflag == 0)
	    copy_c_varsub(fguard, rule, stack_offset);
	  else
	    copy_perl_varsub(fguard, rule, stack_offset);
	  break;

	case '@':
	  if (langflag == 0)
	    copy_c_loc(fguard, stack_offset);
	  else
	    copy_perl_loc(fguard, stack_offset);
	  break;
...
void
copy_c_action(rule, stack_offset)
symbol_list *rule;
int stack_offset;
{
  register int c;
  ...
  while (count > 0)
    {
      while (c != '}')
	    ...
	    case '\'':
	    case '"':
	      copy_quote(faction, c, 0, rule, stack_offset);
	      break;
		...
		continue;
	      ...
	      if (c == '/') /* C++ comment */
		copy_line_comment(faction, NULL);
	      else
		copy_c_comment(faction, NULL);
	      break;

	    case '$':
	      if (langflag == 0)
		copy_c_varsub(faction, rule, stack_offset);
	      else
		copy_perl_varsub(faction, rule, stack_offset);
	      break;

	    case '@':
	      if (langflag == 0)
		copy_c_loc(faction, stack_offset);
	      else
		copy_perl_loc(faction, stack_offset);
	      break;
...
void
copy_perl_action(rule, stack_offset)
symbol_list *rule;
int stack_offset;
{
  register int c;
  ...
  while (count > 0)
    {
      while (c != '}')
	    ...
	    case '"':
	    case '`':
	      copy_quote(faction, c, 1, rule, stack_offset);
	      break;
	  ...
	  /***********/
	    ...
	    case '#':
	      putc(c, faction);
	      copy_line_comment(faction, fdefines);
	      break;
